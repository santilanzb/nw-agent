...
The plan assumes the following. Confirm or override before execution.

1. **Confidence thresholds.** Defaults: 0.80 execute, 0.55 clarify, 0.05 tiebreak margin. Reasonable for OpenAI `text-embedding-3-small`; tune after evaluating real traffic. **Default OK?**
2. **Acknowledgment intent behavior.** When patient sends "okay" / "gracias" / "perfecto" with no question, options: (a) no reply at all, (b) brief "🩵" or "Quedo atenta 🩵". **Pick a or b.**
3. **Seed sourcing.** Plan uses FAQ doc + eval cases + synthetic Spanish paraphrases. If you can export the last 30 days of real WhatsApp messages (even just patient-side text, no PII required for seeding), the classifier will be much better. **Send a CSV/JSONL or proceed with synthetic?**
